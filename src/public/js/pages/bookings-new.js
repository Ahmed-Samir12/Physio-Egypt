...
function setLoading(isLoading) {
  ...
  if (btnText) btnText.textContent = isLoading ? 'جاري الإنشاء…' : 'إنشاء الحجز';
  const sp = submitBtn.querySelector('.spinner');
  if (isLoading && !sp) {
    const s = document.createElement('span'); s.className = 'spinner'; submitBtn.prepend(s);
...
function setFound(isFound) { foundEl?.classList.toggle('hidden', !isFound); }

function pillSync() {
  genderWrap?.querySelectorAll('.pill').forEach(pill => {
    pill.classList.toggle('is-active', Boolean(pill.querySelector('input')?.checked));
  });
}
genderWrap?.addEventListener('change', pillSync);
pillSync();

function fmtEGP(n) { return n > 0 ? `${n} EGP` : '—'; }

function updateRemaining() {
  const rem = Math.max(0, (Number(totalEl?.value) || 0) - (Number(depositEl?.value) || 0));
...
const tryFindPatient = debounce(async () => {
  ...
  try {
    ...
    if (!res.ok) return;
    const list = json?.data?.patients || json?.data?.results || json?.data || json?.patients || [];
    ...
    if (whatsappNumEl) whatsappNumEl.value = p?.whatsappNumber ? p.whatsappNumber.replace(/^\+20/, '') : '';
    if (pnotesEl)  pnotesEl.value  = p?.notes   || '';
    if (patientIdEl) patientIdEl.value = p?.patientId || '';
    const g = String(p?.gender || '').toLowerCase();
    genderWrap?.querySelectorAll('input[name="gender"]').forEach(r => { r.checked = r.value === g; });
    pillSync();
...
form?.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    const payload = {
      patient: {
        ...
        whatsappNumber: whatsappNumEl?.value?.trim()
          ? (whatsappNumEl.value.trim().startsWith('+') ? whatsappNumEl.value.trim() : `+20${whatsappNumEl.value.trim().replace(/^0/, '')}`)
          : '',
    ...
    const id = b?._id || b?.id;
    showAlert('success', 'تم جدولة الموعد بنجاح.', { title: 'تم إنشاء الحجز!' });
