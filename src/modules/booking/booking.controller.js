...
import { generateCardPDF } from '../../utils/patientCard.js';
import Booking from './booking.model.js';
import Patient from '../patient/patientModel.js';

export const createBooking = async (req, res) => {
  const { booking, patientIsNew } = await bookingService.createBooking(req.body, req.user._id);
  res.status(201).json({
    status: 'success',
...
// GET /api/v1/bookings/:id/card?pdf=1  → PDF binary
export const getBookingCard = async (req, res) => {
  ...
  if (req.query.pdf === '1' && pdf) {
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="patient-card-${booking._id}.pdf"`);
    return res.send(pdf);
  }
